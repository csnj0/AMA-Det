...
class ATSS2BHead(BaseDenseHead):
    ...
    def loss(self,
        ...
        for (labels, bbox_targets, cls_scores, bbox_preds, gt_bboxes_, gt_labels_) in zip(
                                                        labels_list,
                                                        bbox_targets_list,
                                                        flatten_cls_scores,
                                                        flatten_bbox_preds,
            ...
            decoded_bbox_preds = distance2bbox(points, bbox_preds)
            bbox_ious = bbox_overlaps(decoded_bbox_preds, gt_bboxes_)
            points_probs = self.points_prob_target(bbox_ious, gt_labels_, points.size(0))

            cls_mask = cls_scores.new_zeros(cls_scores.size())
            cls_mask[pos_inds, labels[pos_inds]] = 1.0
            cls_weight = torch.where(
                                cls_mask.bool(),
                                (points_probs.detach() - cls_scores.sigmoid()).pow(2.0),
            ...
            if num_pos > 0:
                ...
                bbox_weight = cls_scores[pos_inds, labels[pos_inds]].sigmoid().pow(0.5).detach()
                # bbox_weight_2 = 1.0 / (1.0 - bbox_weight)

                loss_bbox = bbox_weight * self.loss_bbox(
                                decoded_bbox_preds[pos_inds],
                                pos_decoded_bbox_targets,
                                reduction_override='none')

                num_total_bbox += bbox_weight.sum().item()
                # num_total_bbox += bbox_weight_2.sum().item()
        ...
        loss_bbox = torch.stack(loss_bbox_list).sum() * dist.get_world_size() / num_total_bbox

        return dict(
            loss_cls=loss_cls,
            loss_bbox=2 * loss_bbox
        )
