...
class BIATSSHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 **kwargs):
        super(BIATSSHead, self).__init__()
    ...
    def loss(self,
        ...
        for (labels, bbox_targets, cls_scores, bbox_preds, centernesses) in zip(
            ...
            cls_weight = torch.where(
                                cls_mask.bool(),
                                cls_scores.new_ones(cls_scores.size()),
                                cls_scores.sigmoid().pow(2.0)
                            )
            loss_cls = cls_weight * F.binary_cross_entropy_with_logits(
                            cls_scores, 
                            cls_mask,
                            reduction='none'
                        )
            loss_cls_list.append(loss_cls.mul(1.0-cls_mask).sum())
            loss_cls_pos_list.append(loss_cls.mul(cls_mask).sum())
            ...
            if num_pos > 0:
                ...
                pos_cls_prob = cls_scores[pos_inds, labels[pos_inds]].float().sigmoid()
                # pos_cls_prob = loss_cls[pos_inds, labels[pos_inds]].neg().exp().float()
                total_prob = pos_cls_prob * loss_bbox.mul(-1.0).exp() / pos_cls_prob.detach().pow(2.0).sum().add(1E-8).sqrt() / math.sqrt(num_pos)
    

                #  pos_cls_prob.detach().pow(2).sum().add(1E-8).sqrt() / num_pos

                # print('{}, {}, {}'.format(
                #     pos_cls_prob.detach().size(),
                #     pos_cls_prob.detach().pow(2).sum().add(1E-8).sqrt(),
                #     pos_cls_prob.detach().pow(2).sum()
                # ))

                # if pos_cls_prob.detach().pow(2).sum().eq(0.0):
                #     print(pos_cls_prob)
                #     print(pos_cls_prob.pow(2))
                #     print(pos_cls_prob.pow(2).sum())
                #     print(cls_scores[pos_inds, labels[pos_inds]])
                #     exit(0)


                loss_prob = total_prob.sum().float().add(1E-8).log().neg()
                # print('{} {} {} {}'.format(
                #     loss_prob,
                #     total_prob.sum(),
                #     num_pos,
                #     cls_scores[pos_inds, labels[pos_inds]].sigmoid()
                # ))
        ...
        num_total_batch = reduce_mean(torch.tensor(num_total_batch).cuda()).item()


        loss_cls = torch.stack(loss_cls_list).sum() / max(num_total_pos, 1)
        loss_cls_pos = torch.stack(loss_cls_pos_list).sum() / max(num_total_pos, 1)
        ...
        # loss_bbox = torch.stack(loss_bbox_list).sum() / max(num_total_pos, 1)

        loss_prob = 1.0 * torch.stack(loss_prob_list).sum() / max(num_total_batch, 1)
