...
class MaskDeformConv2dModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, groups=1, deform_groups=1, norm_cfg=None):
        ...
        self.conv = ModulatedDeformConv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, 
                                          groups=groups, deform_groups=deform_groups, bias=False)
        self.norm_name, norm = build_norm_layer(norm_cfg, out_channels)
        self.add_module(self.norm_name, norm)
        self.act = nn.ReLU(inplace=True)
        constant_init(self.norm, 1, bias=0)
...
class DualDeformModule(nn.Module):
    def __init__(self, channels, points=9, norm_cfg=None):
        ...
        self.cls_conv = ConvModule(channels, channels, 3, padding=1, norm_cfg=norm_cfg)
        self.reg_conv = ConvModule(channels, channels, 3, padding=1, norm_cfg=norm_cfg)
        normal_init(self.cls_conv.conv, std=0.01)
        normal_init(self.reg_conv.conv, std=0.01)
...
class SERIALGFLHead(AnchorHead):
    ...
    def __init__(self,
        ...
        # self.integral_v2 = Integral_v2(0, self.bins-1, bins=self.bins)

        # self.integral = Integral(-(self.bins//2), self.bins//2, bins=self.bins)
        # self.integral_v2 = Integral_v2(-(self.bins//2), self.bins//2, bins=self.bins)

        self.loss_bbox = build_loss(dict(type='GIoULoss', loss_weight=2.0))
        self.loss_cls = build_loss(dict(type='FocalLoss', use_sigmoid=True, gamma=2.0,
    ...
    def forward_single(self, x, scale):
        ...
        bbox_pred_initial = self.points2distance(pts) * 0.0

        # bbox_pred_refine = self.gfl_reg(reg_feat).float()
        bbox_pred_refine = self.gfl_reg(reg_feat)
        # bbox_pred = self.integral_v2(bbox_pred_refine).type_as(x)
        bbox_pred = scale(bbox_pred_refine).float()
        ...
        #     prob = F.softmax(prob, dim=2).type_as(x).reshape(N, -1, H, W)
        #     cls_score = cls_score * self.reg_conf(prob)

        return cls_score, bbox_pred_initial, bbox_pred
    ...
    def loss_single(self, anchors, cls_score, bbox_pred_initial, bbox_pred_refine, labels, label_weights,
        ...
        if len(pos_inds) > 0:
            ...
            # print('bbox: {}, {}'.format(loss_bbox, pos_decode_bbox_pred.mean()))
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred_initial, bbox_pred_refine, stride, anchors in zip(
            ...
            bbox_pred = (bbox_pred_initial.permute(1, 2, 0).reshape(-1, 4) + \
                bbox_pred_refine.permute(1, 2, 0).reshape(-1, 4)) * stride[0]
