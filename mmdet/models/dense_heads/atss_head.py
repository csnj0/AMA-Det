...
class ATSSHead(AnchorHead):
    ...
    @force_fp32(apply_to=("cls_scores", "bbox_preds", "centernesses"))
    def get_results(
        self, cls_scores, bbox_preds, centernesses, gt_bboxes, gt_labels, img_metas
    ):
        ...
        for img_id in range(len(img_metas)):
            ...
            bbox_preds_list = [
                bbox_preds[i][img_id].detach().permute(1, 2, 0).reshape(-1, 4)
                for i in range(num_levels)
            ]
            centernesses_list = [
                centernesses[i][img_id].detach().permute(1, 2, 0).reshape(-1, 1)
                for i in range(num_levels)
            ]
    ...
    def _get_result_single(
        ...
        bbox_list = []
        for score, bbox_pred, center_score, anchor, stride in zip(
            cls_score, bbox_pred, centerness, anchors, strides
        ):
            bbox = self.bbox_coder.decode(anchor, bbox_pred)
        ...
        for gt_bbox, gt_label in zip(gt_bboxes, gt_labels):
            conf_lvl_list = []
            iou_lvl_list = []
            for score, center_score, bbox, feat_size in zip(
                cls_score, centerness, bbox_list, featmap_sizes
            ):
                # conf = score[:, gt_label].sigmoid() * center_score.squeeze()
                conf = score[:, gt_label].sigmoid()
